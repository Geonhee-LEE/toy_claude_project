...
# ─────────────────────────────────────────────────────────────
# Live 참조 모드 비교 (Time vs Lookahead)
# ─────────────────────────────────────────────────────────────

def live_ref_comparison(
    scenario,
    K: int = 256,
    N: int = 20,
    ...
    """Time-based vs Lookahead 실시간 비교 시뮬레이션.

    좌측: Time-based, 우측: Lookahead를 동시에 실행하며
    각 weight strategy를 순차적으로 보여준다.

    Parameters
    ----------
    weight_types : list of str or None
        비교할 가중치 전략. None이면 4종 전체.
    sim_time : float
        시나리오당 시뮬레이션 시간 [s].
    """
    import logging
    ...
    from .benchmark_runner import create_python_controller
    from .cpp_mppi_assembler import CppMPPIAssembler
    from .scenario import LookaheadInterpolator

    if weight_types is None:
        weight_types = ["vanilla", "log", "tsallis", "risk_aware"]

    for wt_idx, weight_type in enumerate(weight_types):
        print(f"\n  ▶ [{wt_idx+1}/{len(weight_types)}] {weight_type} ...")

        # ── 컨트롤러 생성 (C++ 사용 — 양쪽 동일 백엔드) ──
        time_ctrl = CppMPPIAssembler(
            "diff_drive", weight_type, K=K, N=N, seed=seed,
            obstacles=scenario.obstacles,
        )
        look_ctrl = CppMPPIAssembler(
            "diff_drive", weight_type, K=K, N=N, seed=seed,
            obstacles=scenario.obstacles,
        )

        # ── 인터폴레이터 ──
        time_interp = TrajectoryInterpolator(scenario.trajectory[:, :3], scenario.dt)
        look_interp = LookaheadInterpolator(scenario.trajectory, scenario.dt)

        # ── 시뮬레이터 ──
        robot_params = RobotParams()
        sim_config = SimulationConfig(dt=scenario.dt, max_time=sim_time)

        time_sim = Simulator(robot_params, sim_config)
        look_sim = Simulator(robot_params, sim_config)
        time_sim.reset(scenario.initial_state[:3].copy())
        look_sim.reset(scenario.initial_state[:3].copy())
        time_ctrl.reset()
        look_ctrl.reset()
        look_interp.reset()
        ...
        plt.ion()
        fig = plt.figure(figsize=(22, 12))
        title_suffix = f"  ({wt_idx+1}/{len(weight_types)})" if len(weight_types) > 1 else ""
        fig.suptitle(
            f"Time-based vs Lookahead  [{weight_type}]{title_suffix}",
            fontsize=14, fontweight="bold",
        )
        ...
        ax_time.set_title("TIME-BASED", fontsize=12, color="#1f77b4", fontweight="bold")
        ax_time.set_xlabel("x [m]"); ax_time.set_ylabel("y [m]")
        ax_time.grid(True, alpha=0.2); ax_time.set_aspect("equal")

        # 상단 우: Lookahead 궤적
        ax_look = fig.add_subplot(gs[0, 2:4])
        ax_look.set_title("LOOKAHEAD", fontsize=12, color="#2ca02c", fontweight="bold")
        ax_look.set_xlabel("x [m]"); ax_look.set_ylabel("y [m]")
        ax_look.grid(True, alpha=0.2); ax_look.set_aspect("equal")
        ...
        # 중단 우: 풀이 시간
        ax_solve = fig.add_subplot(gs[1, 2:4])
        ax_solve.set_title("Solve Time [ms]", fontsize=10)
        ax_solve.set_xlabel("Step"); ax_solve.grid(True, alpha=0.2)
        ...
        def _make_lines(ax, color):
            ...
            for _ in range(MAX_SAMPLES):
                (sl,) = ax.plot([], [], "-", color=color, alpha=0.05, lw=0.4)
                sample_lines.append(sl)
            (trace_line,) = ax.plot([], [], color=color, lw=2.5, alpha=0.9,
                                    label="Robot")
            (pred_line,) = ax.plot([], [], "c-", lw=2.0, alpha=0.8)
            (best_line,) = ax.plot([], [], "m-", lw=1.2, alpha=0.6)
            robot = patches.Rectangle((0, 0), 0.3, 0.2, angle=0, fill=True,
        ...
        time_lines = _make_lines(ax_time, "#1f77b4")
        look_lines = _make_lines(ax_look, "#2ca02c")

        # 오차/풀이 시간 라인
        (time_err_line,) = ax_err.plot([], [], color="#1f77b4", lw=1, label="Time")
        (look_err_line,) = ax_err.plot([], [], color="#2ca02c", lw=1, label="Lookahead")
        ax_err.legend(fontsize=8)

        (time_solve_line,) = ax_solve.plot([], [], color="#1f77b4", lw=1, label="Time")
        (look_solve_line,) = ax_solve.plot([], [], color="#2ca02c", lw=1, label="Lookahead")
        ax_solve.legend(fontsize=8)
        ...
        # ── 시뮬레이션 루프 ──
        num_steps = int(sim_time / scenario.dt)
        ctrl_N = N
        ctrl_dt = scenario.dt

        time_trace_x, time_trace_y = [], []
        look_trace_x, look_trace_y = [], []
        time_solve_times, look_solve_times = [], []
        time_errors, look_errors = [], []
        ...
        def _update_samples(info, sample_lines, pred_line, best_line, color):
            ...
            if sample_traj is not None and sample_weights is not None:
                ...
                max_w = np.max(sample_weights)
                for rank, idx in enumerate(top_idx):
                    alpha = float(np.clip(sample_weights[idx] / max_w * 0.4,
                                          0.02, 0.4))
                    sample_lines[rank].set_data(
                        sample_traj[idx, :, 0], sample_traj[idx, :, 1])
                    sample_lines[rank].set_alpha(alpha)
        ...
        for step in range(num_steps):
            ...
            # ── Time-based 스텝 ──
            time_state = time_sim.get_measurement()
            time_ref = time_interp.get_reference(
                t, ctrl_N, ctrl_dt, current_theta=time_state[2],
            )
            time_u, time_info = time_ctrl.compute_control(time_state, time_ref)
            time_sim.step(time_u[:2])

            time_trace_x.append(time_state[0])
            time_trace_y.append(time_state[1])
            time_solve_times.append(time_info["solve_time"] * 1000)
            time_pos_err = np.sqrt((time_state[0] - time_ref[0, 0])**2 +
                                    (time_state[1] - time_ref[0, 1])**2)
            time_errors.append(time_pos_err)

            # ── Lookahead 스텝 ──
            look_state = look_sim.get_measurement()
            look_ref = look_interp.get_reference(
                look_state, ctrl_N, ctrl_dt, current_theta=look_state[2],
            )
            look_u, look_info = look_ctrl.compute_control(look_state, look_ref)
            look_sim.step(look_u[:2])

            look_trace_x.append(look_state[0])
            look_trace_y.append(look_state[1])
            look_solve_times.append(look_info["solve_time"] * 1000)
            look_pos_err = np.sqrt((look_state[0] - look_ref[0, 0])**2 +
                                    (look_state[1] - look_ref[0, 1])**2)
            look_errors.append(look_pos_err)

            # ── 시각화 업데이트 (매 2스텝) ──
            if step % 2 == 0:
                # 궤적 트레이스
                time_lines[1].set_data(time_trace_x, time_trace_y)
                look_lines[1].set_data(look_trace_x, look_trace_y)

                # 샘플 궤적
                _update_samples(time_info, time_lines[0], time_lines[2],
                                time_lines[3], "#1f77b4")
                _update_samples(look_info, look_lines[0], look_lines[2],
                                look_lines[3], "#2ca02c")

                # 로봇 위치
                _update_robot(time_state, time_lines[4], time_lines[5], "#1f77b4")
                _update_robot(look_state, look_lines[4], look_lines[5], "#2ca02c")

                # 오차 그래프
                steps_arr = list(range(len(time_errors)))
                time_err_line.set_data(steps_arr, time_errors)
                look_err_line.set_data(steps_arr, look_errors)
                ax_err.set_xlim(0, max(len(steps_arr), 10))
                max_err = max(
                    max(time_errors[-30:], default=1),
                    max(look_errors[-30:], default=1),
                ) * 1.2
                ax_err.set_ylim(0, max_err)

                # 풀이 시간
                time_solve_line.set_data(steps_arr, time_solve_times)
                look_solve_line.set_data(steps_arr, look_solve_times)
                ax_solve.set_xlim(0, max(len(steps_arr), 10))
                max_solve = max(
                    max(time_solve_times[-30:], default=1),
                    max(look_solve_times[-30:], default=1),
                ) * 1.5
                ax_solve.set_ylim(0, max_solve)

                # 정보 텍스트
                time_avg = np.mean(time_solve_times) if time_solve_times else 0
                look_avg = np.mean(look_solve_times) if look_solve_times else 0
                time_rmse = np.sqrt(np.mean(np.array(time_errors)**2)) \
                    if time_errors else 0
                look_rmse = np.sqrt(np.mean(np.array(look_errors)**2)) \
                    if look_errors else 0

                info_str = (
                    f"Step {step:4d}/{num_steps}  │  Time {t:.2f}s / {sim_time:.0f}s  │  "
                    f"Strategy: {weight_type}  │  K={K}  N={N}  │  "
                    f"[{wt_idx+1}/{len(weight_types)}]\n"
                    f"───────────────────────────────────────────"
                    f"──────────────────────────────────────────\n"
                    f"  TIME-BASED:  v={time_u[0]:+.3f} m/s  "
                    f"ω={time_u[1]:+.3f} rad/s  │  "
                    f"solve={time_info['solve_time']*1000:.2f}ms  "
                    f"ESS={time_info.get('ess',0):.0f}/{K}  "
                    f"pos_err={time_pos_err:.4f}m  "
                    f"RMSE={time_rmse:.4f}m  "
                    f"avg={time_avg:.2f}ms\n"
                    f"  LOOKAHEAD:   v={look_u[0]:+.3f} m/s  "
                    f"ω={look_u[1]:+.3f} rad/s  │  "
                    f"solve={look_info['solve_time']*1000:.2f}ms  "
                    f"ESS={look_info.get('ess',0):.0f}/{K}  "
                    f"pos_err={look_pos_err:.4f}m  "
                    f"RMSE={look_rmse:.4f}m  "
                    f"avg={look_avg:.2f}ms\n"
                    f"───────────────────────────────────────────"
                    f"──────────────────────────────────────────\n"
                    f"  RMSE improvement: "
                    f"{(time_rmse - look_rmse) / time_rmse * 100:.1f}%"
                    f" (lookahead wins)" if look_rmse < time_rmse else
                    f"  RMSE comparison: "
                    f"time={time_rmse:.4f}  look={look_rmse:.4f}"
                )
                info_text.set_text(info_str)
            ...
            # 양쪽 모두 궤적 완료 시
            look_idx, look_dist = look_interp.find_closest_point(look_state[:2])
            time_idx, time_dist = time_interp.find_closest_point(time_state[:2])
            if (look_idx >= look_interp.num_points - 1 and look_dist < 0.1 and
                    time_idx >= time_interp.num_points - 1 and time_dist < 0.1):
                break

        # ── 완료 ──
        time_avg = np.mean(time_solve_times)
        look_avg = np.mean(look_solve_times)
        time_rmse = np.sqrt(np.mean(np.array(time_errors)**2))
        look_rmse = np.sqrt(np.mean(np.array(look_errors)**2))
        improvement = (time_rmse - look_rmse) / time_rmse * 100 \
            if time_rmse > 0 else 0

        final_str = (
            f"━━━━━ COMPLETE: {weight_type} ({step+1} steps) ━━━━━\n\n"
            f"  TIME-BASED:  avg_solve = {time_avg:.2f} ms  │  "
            f"RMSE = {time_rmse:.4f} m\n"
            f"  LOOKAHEAD:   avg_solve = {look_avg:.2f} ms  │  "
            f"RMSE = {look_rmse:.4f} m\n"
            f"  Improvement: {improvement:.1f}%\n\n"
        )
        if wt_idx < len(weight_types) - 1:
            final_str += f"  Close window for next: {weight_types[wt_idx+1]}"
        else:
            final_str += "  All strategies complete. Close window to exit."
