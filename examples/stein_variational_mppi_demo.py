#!/usr/bin/env python3
"""Stein Variational MPPI (SVMPC) 데모 — SVGD iteration수별 비교.

SVGD iteration 수에 따른 샘플 다양성 및 추적 성능 비교.

실행:
    python examples/stein_variational_mppi_demo.py
    python examples/stein_variational_mppi_demo.py --trajectory circle
    python examples/stein_variational_mppi_demo.py --iterations 0 1 3 5
    python examples/stein_variational_mppi_demo.py --live
    python examples/stein_variational_mppi_demo.py --save comparison.png
    python examples/stein_variational_mppi_demo.py --no-obstacles
    python examples/stein_variational_mppi_demo.py --no-plot
"""
...
class RunResult:
    ...
    diversity_before: np.ndarray
    diversity_after: np.ndarray
...
def simulate(
    ...
    div_before_list, div_after_list = [], []
    ...
    for step in range(num_steps):
        ...
        div_before_list.append(info.get("sample_diversity_before", 0))
        div_after_list.append(info.get("sample_diversity_after", 0))
    ...
    return RunResult(
        name=name, svgd_iters=svgd_iters,
        ...
        diversity_before=np.array(div_before_list),
        diversity_after=np.array(div_after_list),
...
def print_summary(results: List[RunResult]) -> None:
    ...
    lines = [
        "", sep,
        "|{:^{w}s}|".format("Stein Variational MPPI (SVMPC) 비교", w=30 + col_w * n + 2),
        sep,
    ...
    metrics = [
        ...
        ("Avg Div Before", lambda r: f"{np.mean(r.diversity_before):>{col_w}.4f}"),
        ("Avg Div After", lambda r: f"{np.mean(r.diversity_after):>{col_w}.4f}"),
...
def plot_comparison(
    ...
    for r in results:
        c = ITER_COLORS.get(r.svgd_iters, "tab:gray")
        ax.plot(r.time_array, r.diversity_after, color=c, lw=1, label=f"{r.name} (after)")
        ax.plot(r.time_array, r.diversity_before, color=c, lw=1, ls=":", alpha=0.4)
    ax.set_xlabel("Time [s]"); ax.set_ylabel("Diversity")
    ax.set_title("Sample Diversity (solid=after, dot=before)")
    ...
    for r in results:
        c = ITER_COLORS.get(r.svgd_iters, "tab:gray")
        ax.plot(r.time_array, r.solve_times * 1000, color=c, lw=1, alpha=0.7, label=r.name)
    ax.set_xlabel("Time [s]"); ax.set_ylabel("Solve Time [ms]")
    ax.set_title("Solve Time"); ax.legend(fontsize=7); ax.grid(True, alpha=0.3)

    fig.suptitle("Stein Variational MPPI (SVMPC): iteration comparison",
...
def main():
    parser = argparse.ArgumentParser(description="Stein Variational MPPI (SVMPC) iteration 비교 데모")
    ...
    parser.add_argument("--iterations", type=int, nargs="*", default=None,
                        help="비교할 SVGD iteration 수 (기본: 0 1 3 5)")
    ...
    iter_values = args.iterations if args.iterations else [0, 1, 3, 5]
