"""Model Predictive Controller for swerve drive path tracking."""
...
@dataclass
class SwerveMPCParams:
    """MPC tuning parameters for swerve drive."""
    ...
    def __post_init__(self):
        ...
        if self.R is None:
            self.R = np.diag([0.1, 0.1, 0.1])  # 3 control inputs for swerve
        if self.Qf is None:
            self.Qf = np.diag([100.0, 100.0, 10.0])
        if self.Rd is None:
            self.Rd = np.diag([0.5, 0.5, 0.5])  # 3 control inputs for swerve


class SwerveMPCController:
    """
    Nonlinear MPC controller for swerve drive path tracking.
    ...
    def __init__(
        self,
        robot_params: SwerveParams | None = None,
        mpc_params: SwerveMPCParams | None = None,
        log_file: str | Path | None = None,
    ):
        self.robot = SwerveDriveModel(robot_params or SwerveParams())
        self.params = mpc_params or SwerveMPCParams()
    ...
    def _setup_logging(self, log_file: str | Path | None = None) -> None:
        ...
        if log_file is not None:
            ...
            logger.info(f"Swerve MPC logging to file: {log_file}")

    def _setup_optimizer(self) -> None:
        """Setup the NLP optimization problem."""
        N = self.params.N
        nx = SwerveDriveModel.STATE_DIM
        nu = SwerveDriveModel.CONTROL_DIM
        ...
        self.solver = ca.nlpsol("swerve_mpc_solver", "ipopt", nlp, opts)
        self.lbx = lbx
        self.ubx = ubx
    ...
    def compute_control(
        ...
        logger.info(
            f"Swerve MPC iteration {self._iteration_count}: "
    ...
    def reset(self) -> None:
        ...
        self.prev_solution = None
        self._iteration_count = 0
        logger.debug("Swerve MPC controller reset")
